...
import { COOKIE_DEFAULT_SECTIONS, resolveSections } from "../data/pageDefaults";
...
function CookiePage() {
  const [sections, setSections] = useState(COOKIE_DEFAULT_SECTIONS);

  useEffect(() => {
    fetchPageContent("cookie")
      .then((page) => setSections(resolveSections(COOKIE_DEFAULT_SECTIONS, page.sections, page.updatedAt)))
      .catch(() => {});
  }, []);

  return (
    <div style={{ fontFamily: "Raleway, sans-serif", minHeight: "70vh" }}>
      <Seo title="Cookie Policy" description="Informativa sui cookie di Liguria Design Group ai sensi del GDPR (Reg. UE 2016/679)." />
