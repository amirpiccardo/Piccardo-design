...
const ContractBrand = require("../models/ContractBrand");
...
router.get("/", async (req, res) => {
  try {
    const brands = await ContractBrand.find();
...
router.put("/:id", authMiddleware, adminMiddleware, upload.single("logo"), async (req, res) => {
  ...
  try {
    const brand = await ContractBrand.findByIdAndUpdate(req.params.id, updateData, { new: true });
...
router.delete("/:id", authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const brand = await ContractBrand.findByIdAndDelete(req.params.id);
    if (!brand) return res.status(404).json({ message: "Brand non trovato" });
